    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['interview:position:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['interview:position:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['interview:position:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['interview:position:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="positionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="岗位名称" align="center" prop="name">
        <template slot-scope="scope">
          <el-button type="text" @click="goToDimension(scope.row.name)">
            {{ scope.row.name }}
          </el-button>
        </template>
        </el-table-column>
...
<script>
import { listPosition, getPosition, delPosition, addPosition, updatePosition } from "@/api/interview/position"
...
export default {
  ...
  methods: {
    //跳转页面
    goToDimension(positionName) {
  // 跳转到维度管理页面，并传入岗位ID和名称作为查询参数
    this.$router.push({
    path: '/interview/dimension',
    query: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('interview/position/export', {
        ...this.queryParams
