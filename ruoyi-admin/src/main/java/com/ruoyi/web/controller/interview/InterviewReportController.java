...
public class InterviewReportController extends BaseController
{
    ...
    @Log(title = "面试报告", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public void export(HttpServletResponse response, InterviewReport interviewReport)
    {
        ...
        util.exportExcel(response, list, "面试报告数据");
    }
...
//    @PreAuthorize("@ss.hasPermi('interview:report:query')")
    ...
    @Log(title = "面试报告", businessType = BusinessType.INSERT)
    @PostMapping
    ...
    @Log(title = "面试报告", businessType = BusinessType.UPDATE)
    @PutMapping
    ...
    @Log(title = "面试报告", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
